...
class TableAgent:
    ...
    def query(self, prompt) -> str:
        ...
        # Fallback for providers that do not return usable API usage.
        if total_token_count <= 0:
            input_token_count = estimated_input_token_count
            output_token_count = self.model.get_token_count(
                response_text
            )
            total_token_count = (
    ...
    def solver_loop(self, df: pd.DataFrame, prompt: str) -> str:
        ...
        for i in range(self.max_depth):
            ...
            if 'Action:' not in text:
                ...
                execution_globals = {**globals(), 'df': df, **memory}
